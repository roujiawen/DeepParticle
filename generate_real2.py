...
res=30 # resolution of the cell, total size = res^2
treshold = 0.01 # treshold for droping cell which are almost never used
masscuts=[120,220]#[120,220]#[120,220]#[150,190]#[130,200]#[0,1e5]  #mass cuts
...
def get_histo(lead, comp, resolution,rotation_type=0):  # function that gets a histogram based on the events
    ...
    H, xedges, yedges = np.histogram2d(phi,eta,resolution,[[-pi/2.0, pi/2.0], [-pi/2.0, pi/2.0]],weights=weights_jet)
...
#Source files
top_file='ttbar_R_1.0_pt_800_900_m_130_210.txt'#'ttbar_R_1.0_pt_500_600_m_130_200.txt'##'ttbar_R_1.0_pt_500_600_m_130_200.txt'#'ttbar_no_matching_pt500-600_m150-190_add.txt' # 'ttbar_no_matching_pt500-600_m150-190.txt'
dijet_file='qcd_R_1.0_pt_800_900_m_130_210.txt'#'qcd_R_1.0_pt_500_600_m_130_200.txt'#'qcd_R_1.0_pt_500_600_m_130_200.txt'#'dijets_pt_500-600_m150-190_np_add.txt' #  'dijets_pt_500-600_m150-190_np.txt'
...
print 'number of top jets', np.shape(leading_top)[0]
print 'number of dijets', np.shape(leading_dijet)[0]

### Get the Histogram and feature vectors
H_top=[]
H_top_feature=[]
for j in range(np.shape(leading_top)[0]):
    H_top.append(get_histo(leading_top[j], components_top[j], res,rotation_type=1))
...
for j in range(np.shape(leading_dijet)[0]):
    H_dijet.append(get_histo(leading_dijet[j], components_dijet[j], res,rotation_type=1))
...
np.save('H_top_feature_aligned_new_800_30_clean01_test', H_top_feature)
np.save('H_dijet_feature_aligned_new_800_30_clean01_test', H_dijet_feature)

np.save('H_top_mass_800_full', mass_top)
np.save('H_dijet_mass_800_full', mass_dijet)
